...
2. **Her çözüm kendi `#ifdef` bloğunda** (`ENABLE_FIX_UART_PRIO`, `ENABLE_FIX_BTN_PRIO`), çalışma anında `fix_mask` ile seçiliyor. Arayüzde 4 seçenek var: standart (0), naif (F4), kök neden (F1), optimal (F1+F4).
...
4. **Önerilen çözüm:** F1 + F4, yani UartTxTask ve ButtonTask'ın ikisi de CPU işinin üstünde (öncelik 4).
...
- **Denenip çıkarılanlar:** Üç varyant daha ölçüldü ve tekrar ettikleri için kaldırıldı:
  ...
  - **Sonraki gönderimi TC kesmesinden başlatmak:** F1 ile aynı sonucu verdi.
